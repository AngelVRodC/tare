...
`tare tools --harness opencode`, and [Codex](#codex) via
...
`--harness` is `tools` only: which harness to read, claude-code (default),
opencode or codex. Registered there and nowhere else, so `tare scan --harness opencode`
